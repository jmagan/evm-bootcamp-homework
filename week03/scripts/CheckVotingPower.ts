import { formatEther, parseEther, Hex, hexToString, http } from "viem";
import {
  abi,
  bytecode,
...
async function main() {
  ...
  if (!parameters || parameters.length < 1)
    throw new Error(
      "Required contract address parameter not provided. Required: <contract address> Optional: <wallet address>"
    );
