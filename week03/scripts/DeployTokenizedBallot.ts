...
async function main() {
    ...
    if (!parameters || parameters.length < 3)
        throw new Error("Required parameters not provided: <token contract address> <target block number> <proposal names>");
    ...
    const hash = await deployer.deployContract({
        ...
        args: [proposals.map((prop) => toHex(prop, { size: 32 })), tokenContractAddress, targetBlock],
    });
    console.log("Transaction hash: ", hash);
    console.log("Waiting for confirmations...");
    const receipt = await publicClient.waitForTransactionReceipt({ hash });
    console.log("TokenizedBallot contract deployed to address: ", receipt.contractAddress);
